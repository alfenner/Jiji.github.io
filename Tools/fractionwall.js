...
function newTexture(self,color) {
  ...
  graphics.endFill();
  return wall.renderer.generateTexture(graphics);
}
...
function initTickArray(d) {
  for (var i = 0;i<=12;i++){
    ...
    wall.stage.addChild(tick);
    tickArray.push(tick)
    createjs.Tween.get(tick).to({x: wallWidth/d*i}, 1000, createjs.Ease.getPowInOut(4))
  }
}
...
function animateTicks(den) {
    tickArray.forEach((t,i) => {
      if (i > den){
        createjs.Tween.get(t).to({x: wallWidth}, 1000, createjs.Ease.getPowInOut(4))
      } else {
        createjs.Tween.get(t).to({x: wallWidth/den*i}, 1000, createjs.Ease.getPowInOut(4))
      }
    })
  }
...
function createTile(x,y,w,h,d) {
  ...
  graphics.endFill();
    ...
    var texture = wall.renderer.generateTexture(graphics);
